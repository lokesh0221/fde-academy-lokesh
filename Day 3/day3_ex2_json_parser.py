...
def extract_shipment_record(shipment: dict) -> dict:
    ...
    Uses .get() with defaults for all nested field access — never raises KeyError.
    """
...
def parse_api_response(response: dict) -> list[dict]:
    ...
    Returns empty list if "shipments" key is missing.
    """
...
def compute_carrier_summary(records: list[dict]) -> list[dict]:
    ...
    Returns list of dicts sorted by total_revenue descending.
    """
    stats: dict[str, dict] = {}
    for r in records:
        code = r["carrier_code"]
        ...
        s = stats[code]
        s["shipment_count"] += 1
        if r["charge_total"] is not None:
            s["total_revenue"] += r["charge_total"]
    ...
    for s in stats.values():
        avg = (
            round(s["total_delay"] / s["shipment_count"], 1)
            if s["shipment_count"]
            else 0.0
        )
        result.append(
...
if __name__ == "__main__":
    ...
    for row in summary:
        print(
            ...
            f"revenue=Rs.{row['total_revenue']:,.2f}",
