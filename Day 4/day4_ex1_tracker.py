...
class ShipmentTracker:
    ...
    def delay_penalty(self, rate: Optional[float] = None) -> float:
        ...
        r = rate if rate is not None else self.PENALTY_RATE_PER_DAY
        return round(self._delay_days * r, 2)
    ...
    def status_history(self) -> list[str]:
        ...
        return [
            f"{frm} -> {to} @ {ts.isoformat()}"
            for frm, to, ts in self._history
        ]
...
if __name__ == "__main__":
    ...
    shipments = [s, s2, ShipmentTracker("sh-003", dhl, "Pune", "Hyderabad", delay_days=0)]
